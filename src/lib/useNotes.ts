  ...
  const loadNotes = async () => {
    ...
    if (s) {
      all = all.filter((n) =>
        ...
        ((n.sourceUrl||'') as string).toLowerCase().includes(s) ||
        (Array.isArray(n.topics) && n.topics.join(' ').toLowerCase().includes(s))
      );
    }
    if (filters.favorite) all = all.filter((n) => !!n.favorite);
    if (filters.topics?.length) all = all.filter((n) => Array.isArray(n.topics) && filters.topics.every((t: string) => n.topics.includes(t)));
    setNotes(all.sort((a,b)=> (b.updatedAt||b.createdAt||0) - (a.updatedAt||a.createdAt||0)));
  };
