...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            # 출력 형식 (반드시 준수)
            - 단일 JSON 객체로만 응답합니다.
            - `content` 필드는 마크다운 형식이며, 수식은 LaTeX를 사용합니다.
            {{
            ...
            - **알고리즘, 프로세스, 의사결정 흐름** 설명 시: **순서도(Flowchart)**를 사용합니다.
            - **시스템 구성요소 간 상호작용** 설명 시: **시퀀스 다이어그램(Sequence Diagram)**을 사용합니다.
            - **객체 지향 프로그래밍(OOP) 구조** 설명 시: **클래스 다이어그램(Class Diagram)**을 사용합니다.
            - **시스템의 상태 변화** 설명 시: **상태 다이어그램(State Diagram)**을 사용합니다.
            - **프로젝트 일정이나 타임라인** 설명 시: **간트 차트(Gantt Chart)**를 사용합니다.
            - 다이어그램 코드는 반드시 ```mermaid ... ``` 코드 블록으로 감싸야 합니다.

            # 작업 순서
            1. **문제 분석:** 문제의 핵심 요소를 파악합니다.
            2. **핵심 개념 정리:** 문제 해결에 필요한 이론과 공식을 정리합니다.
            3. **모범 풀이:** 위 '시각 자료 활용 규칙'을 적극적으로 사용하여 단계별로 상세히 설명합니다.
