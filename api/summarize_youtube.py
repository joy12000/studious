...
- **가독성 향상 (마크다운 사용):**
  - 중요한 키워드나 문장은 `**굵은 글씨**`로 강조해주세요.
  - 내용상 목록화가 필요한 경우, `- 글머리 기호`를 사용하여 명확하게 구분해주세요.
  - 문단과 문단 사이는 줄바꿈을 확실히 하여 시각적으로 분리되도록 작성해주세요.
  - **매우 중요:** `summary` 필드 내의 마크다운 내용은 JSON 문자열 내부에 포함되므로, 모든 특수 문자(예: 줄바꿈 `\n`)는 JSON 규칙에 따라 적절히 이스케이프 처리(`\\n`)되어야 합니다.

[결과 출력 형식]
아래와 같은 JSON 형식에 맞춰 한국어로 결과를 반환해주세요.
**매우 중요:** `summary` 내용이 길어지더라도, 반드시 전체 JSON 구조가 깨지지 않고 완벽하게 끝나도록 작성해야 합니다.
{
  "summary": "마크다운 서식이 적용된, 영상의 길이를 고려하여 분량이 조절된 상세하고 체계적인 핵심 요약문",
  "key_insights": [
