...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            4.  **이미지 (Image):** 학습 자료에 `![](URL)` 형식의 마크다운 이미지나 이미지 URL이 포함된 경우, 이를 **반드시** `<img src="URL"/>` HTML 태그로 변환하여 본문에 포함해야 합니다. 원본의 이미지 컨텍스트를 유지하여 적절한 위치에 배치하세요.
                -   예시: `![](https://example.com/image.jpg)`는 `<img src="https://example.com/image.jpg"/>`로 변환합니다.
