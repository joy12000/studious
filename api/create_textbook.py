...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            4.  **이미지 (Image):** 학습 자료에 `![](URL)` 형식의 이미지가 있다면, **설명을 보충하거나 이해를 돕는 데 필수적이라고 판단될 경우에만** 해당 이미지를 `<img src="URL"/>` HTML 태그로 변환하여 본문에 포함하세요. 이미지를 사용할 때는, 왜 이 이미지가 중요한지에 대한 설명을 덧붙여 학습 효과를 극대화해야 합니다.
