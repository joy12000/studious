...
class handler(BaseHTTPRequestHandler):
    ...
    def get_system_prompt(self, note_context):
        ...
        # 📝 노트 수정 제안 규칙
        **중요: 사용자가 '수정해줘', '고쳐줘', '다듬어줘', '제안해줘' 등 명시적으로 수정을 요청하는 경우에만 아래 suggestion 형식을 사용하세요.** 그 외의 경우에는 절대로 먼저 수정 제안을 하지 마세요.
        수정을 제안할 경우, **절대로, 반드시** 다음 형식을 사용해야 합니다. 이 규칙을 어길 시, 응답은 실패로 간주됩니다.
        ```suggestion
        기존 내용
        ===>
        새로운 내용
        ...
        ## 💡 수정 제안 예시
        ### 사용자 질문:
        "노트 내용이 좀 부족한 것 같아. 베르누이 방정식 부분을 더 자세하게 다듬어줘."
        ### 노트 내용:
        "베르누이 방정식은 유체의 속도와 압력의 관계를 나타낸다."
        ### AI 답변 예시:
        "네, 알겠습니다. 노트의 해당 부분을 더 정확하고 상세한 내용으로 다듬어 드릴게요.
        ```suggestion
        기존 내용
        베르누이 방정식은 유체의 속도와 압력의 관계를 나타낸다.
        ===>
        ...
        ```
        이렇게 수정하면 개념을 더 정확하게 이해하는 데 도움이 될 것입니다."
